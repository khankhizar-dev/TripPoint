...
class ChangePasswordViewModel(
    ...
    override fun onIntent(intent: ChangePasswordContract.Intent) {
        when (intent) {
            is ChangePasswordContract.Intent.CurrentPasswordChanged -> setState { 
                copy(currentPassword = intent.value, currentError = null) 
            }
            is ChangePasswordContract.Intent.NewPasswordChanged -> setState { 
                copy(newPassword = intent.value, newError = null) 
            }
            is ChangePasswordContract.Intent.ConfirmPasswordChanged -> setState { 
                copy(confirmPassword = intent.value, confirmError = null) 
            }
            ChangePasswordContract.Intent.ToggleCurrentVisibility -> setState { 
                copy(isCurrentVisible = !isCurrentVisible) 
            }
            ChangePasswordContract.Intent.ToggleNewVisibility -> setState { 
                copy(isNewVisible = !isNewVisible) 
            }
            ChangePasswordContract.Intent.ToggleConfirmVisibility -> setState { 
    ...
    private fun savePassword() {
        ...
        viewModelScope.launch {
            ...
            if (result.isSuccess && result.getOrDefault(false)) {
                ...
                sendEffect(ChangePasswordContract.Effect.ShowError(errorMsg))
            }
        }
    }
}
